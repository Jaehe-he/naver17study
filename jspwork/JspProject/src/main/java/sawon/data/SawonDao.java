...
public class SawonDao {
	...
	public List<SawonDto> getAllDatas(String search) { //리스트 안에 데이터가 들어있는 SawonDto를 묶어서 반환하겠다.
		...
		try {
			...
			while(rs.next()) {
				...
				//dto에 레코드단위의 데이터를 다 넣은 후 list에 추가
				list.add(dto);
			}
	...
	public void insertSawon(SawonDto dto) {
		...
		public void updateSawon(SawonDto dto) {
			...
			String sql = """
					update sawon set sname=?, sphoto=?, ipsaday=?, sbirth=?, sblood=?
					where num=?
					""";
			conn = connect.getConnection();
			try {
