...
-- WRITEDAY가 NULL일 경우 '2024-12-12'로 변경을 해보자
UPDATE SAWON SET WRITEDAY='2024-12-12' WHERE WRITEDAY IS NULL;

COMMIT;

-- 삭제 DELETE
DELETE FROM SAWON; -- WHERE 조건을 안쓴경우 전체 데이터가 삭제된다
...
CREATE TABLE BOOKING (
    ...
    BHP VARCHAR2(20) CONSTRAINT UQ_BOOKING_BHP UNIQUE, --BOOKING하는 사람의 핸드폰번호
    FOODNUM NUMBER(3),
    BOOKINGDAY DATE,
...
-- 오류 발생시켜보기
INSERT INTO BOOKING VALUES (SEQ_FOOD.NEXTVAL, '김말자', '010-777-5678', 401, '2025-01-10'); --401은 메뉴에 없음
...
FROM FOOD F, BOOKING B
WHERE F.FOODNUM=B.FOODNUM(+); --이때 아무도 주문하지 않은 메뉴는 주문자가 NULL이 나온다

-- 위의 SQL문을 이용해서 주문자 이름을 빼고 NULL 인경우만 출력하면 아무도 주문하지 않은
-- 메뉴만 골라낼 수 있다
SELECT
...
-- 자식 테이블 생성 시 ON DELETE CASCADE 설정을 안했을경우는 못지운다
DELETE FROM FOOD WHERE FOODNUM=200; --200번 메뉴는 이영자가 예약주문함. 그래서 삭제못함
-- 오류메시지 : 무결성 제약조건(ANGEL.FK_FOODNUM)이 위배되었습니다. 자식 레코드가 발견되었습니다.
