...
SELECT ENAME, to_char(hiredate, 'yyyy-mm-dd') 입사일, SAL FROM EMP WHERE to_char(hiredate, 'mm') = 05;
-- SELECT ENAME, to_char(hiredate, 'mm') 입사일, SAL FROM EMP WHERE to_char(hiredate, 'mm') = 05;
...
-- SELECT ENAME, JOB, SAL FROM EMP WHERE ENAME LIKE IN ('A%', 'S%', 'M%'); -- 왜 안되는지 모르겠음
